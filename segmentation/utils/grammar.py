...
class PathGrammar(Grammar):
    ...
    def __init__(self, path):
        ...
        #print("line 37 grammar ",label2index_map)
        transcripts = self._read_transcripts(path)
        # generate successor sets
        self.successors = dict()
        for transcript in transcripts:
            transcript = transcript + [self.end_symbol()]
            #print("line 42 grammar ",transcript)
    ...
    def _read_transcripts(self,path):
        transcripts = []
        transcript_file = json.load(open(path,"r"))
        for rep, trans in transcript_file.items():
