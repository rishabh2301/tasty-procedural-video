...
class Buffer(object):
    ...
    def __init__(self, buffer_size, n_classes):
        ...
        #self.embd_classes = embd_classes
    ...
    def add_sequence(self, features, transcript, framelabels):
        if len(self.features) < self.buffer_size:
            ...
            #print("network line 33 ",transcript)
            self.framelabels.append(framelabels)
            #print("network line 35 ",framelabels)
...
class Trainer(Forwarder):
    ...
    def train(self, sequence, transcript, batch_size = 50, learning_rate = 0.1):
        data_wrapper = DataWrapper(sequence, window_size = 21)
        ...
        sequence_loss = 0
        #print("i2e",index2embd)
        for i, data in enumerate(dataloader, 0):
            optimizer.zero_grad()
            input, target = data
            #print("215 input size",input.shape)
            #print("215 target size",target.shape)
            input = Variable(input.cuda())
            ...
            output = self.net(input)
            #print("215 output size",output.shape)
            #print("output is ",output.shape)
            output = output[0, :, :]
            #print("215 output size",output.shape)
            #print("output2 is ",output.shape)
